...
$.PikaChoose=function(e,o){
    ...
    $pc.fn.extend({
        build:function(){
            ...
            }else{this.imgPlay.addClass('play');}
        ...
        bindEvents:function(){
            ...
            this.wrap.bind('mouseenter',{self:this},function(e){e.data.self.imgPlay.stop(true,true).fadeIn('fast');});
            this.wrap.bind('mouseleave',{self:this},function(e){e.data.self.imgPlay.stop(true,true).fadeOut('fast');});
            this.tooltip.bind('mouseenter',{self:this},function(e){clearTimeout(e.data.self.stillOut);});
            this.tooltip.bind('mouseleave',{self:this},function(e){e.data.self.stillOut=setTimeout(e.data.self.hideTooltip,700);});
        },
        ...
        imgClick:function(e,x){
            ...
            if(typeof(x)=='undefined'||x.how!="auto"){
                if(self.options.autoPlay){self.imgPlay.trigger('click');}
            }
            self.animating=true;
            ...
            switch(n){
                case 0:
                    ...
                    self.image.fadeOut('slow',function(){
                        self.image.attr('src',data.source).fadeIn('slow',function(){self.finishAnimating(data);
                        });
                    });
                    break;
                case 1:
                    ...
                    self.aniDiv.children().hide().each(function(index){
                        ...
                        $(this).css({opacity:0.1}).show().delay(delay).animate({
                            ...
                            "height":aHeight},
                                ...
                                'linear',
                                function(){
                                    if($(".animation div").index(this)==24){self.finishAnimating(data);}
                                });
                    });
                    break;
                case 3:
                ...
                case 4:
                    self.aniDiv.children().hide().each(function(index){
                        ...
                        $(this).css({opacity:0.1,"height":aHeight}).show().delay(delay).animate({
                            opacity:1,
                            "width":aWidth},
                                800,
                                'linear',
                ...
                case 5:
                    self.aniDiv.children().show().each(function(index){
                        ...
                        aHeight=self.gapper($(this),aHeight);
                        if($(".animation div").index(this)==24){delay=800;}
                        $(this).css({
                            "height":aHeight,
                            "width":aWidth,
                            "opacity":.01}).delay(delay).animate({
                            "opacity":1},
                                800,
                                function(){
                                    if($(".animation div").index(this)==24){self.finishAnimating(data);}
                ...
                case 6:
                    ...
                    self.aniDiv.children('div').hide();
                    self.aniDiv.css({width:0}).show().animate({
                        width:self.image.width()},
                            'slow',
                            function(){
                                self.finishAnimating(data);
        ...
        finishAnimating:function(data){
            ...
            if(this.options.showCaption){this.caption.html(data.caption).fadeIn('slow');}
